...
void seff_coroutine_delete(seff_coroutine_t *k) {
    ...
    // 释放协程的栈（使用我们自定义的 release_stack_frame）
    release_stack_frame(k->frame_ptr);
    // 协程结构本身也是用 mmap 分配的，使用 munmap 释放
    if (munmap(k, sizeof(seff_coroutine_t)) != 0) {
        perror("munmap coroutine failed");
    }
#else
    seff_coroutine_release(k);
    free(k);
...
void seff_coroutine_release(seff_coroutine_t *k) {
    STACK_POLICY_SWITCH(
        ...
        }, 
        {
         ...
         //munmap(k->frame_ptr, sizeof(seff_coroutine_t));
         release_stack_frame(k->frame_ptr);
